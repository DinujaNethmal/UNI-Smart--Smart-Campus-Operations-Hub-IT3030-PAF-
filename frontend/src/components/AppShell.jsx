...
import {
  ...
  GraduationCap, Bell, ChevronDown, LogOut, Menu, Building2, CheckCheck, X, Ticket
} from 'lucide-react';

import { useAuth } from '../hooks/useAuth';
import { getMyNotifications, getUnreadNotifications, markNotificationAsRead } from '../api/notificationApi';
...
export default function AppShell() {
  ...
  useEffect(() => {
    ...
    async function loadNotificationsSnapshot() {
      ...
      try {
        ...
        const nextIds = new Set(all.map((item) => item.id));
        if (hasLoadedNotificationsRef.current) {
          const freshUnread = all.filter(
            (item) => !seenNotificationIdsRef.current.has(item.id) && !item.read
          );
    ...
    const intervalId = window.setInterval(loadNotificationsSnapshot, 5000);
  ...
  const initials = user?.name?.split(' ').map((word) => word[0]).join('').slice(0, 2) || 'GU';
  ...
  return (
    <div className="app-shell">
      ...
      <header className="topbar">
        <button className="topbar-icon-btn" style={{ display: 'none' }}>
          <Menu size={18} />
        </button>
        ...
        <div className="topbar-actions">
          <div ref={notificationRef} style={{ position: 'relative' }}>
            <button className="topbar-icon-btn" onClick={() => setNotificationMenuOpen((open) => !open)}>
              <Bell size={18} />
              {unreadCount > 0 && (
            ...
            {notificationMenuOpen && (
              <div className="notification-dropdown">
                <div className="notification-dropdown-header">
                  ...
                  {unreadCount > 0 && <span className="notification-dropdown-pill">{unreadCount} unread</span>}
                </div>
                    ...
                    recentNotifications.map((item) => (
                      ...
                      >
                        <div className="notification-dropdown-item-top">
                          ...
                          <span className="notification-dropdown-item-time">{formatNotificationTime(item.createdAt)}</span>
                        </div>
                        <div className="notification-dropdown-item-type">{formatNotificationType(item.type)}</div>
          ...
          {!loading && isAuthenticated ? (
            <>
              <div ref={dropRef} style={{ position: 'relative' }}>
                <button className="topbar-user" onClick={() => setMenuOpen((open) => !open)}>
                ...
                {menuOpen && (
                  <div className="role-dropdown">
                    ...
                    <div className="role-dropdown-footer">Signed in with {user.provider}</div>
                  </div>
                )}
              </div>
              <button
            ...
            <button className="topbar-auth-btn" onClick={() => navigate('/login')}>Login</button>
          )}
        </div>
      </header>
